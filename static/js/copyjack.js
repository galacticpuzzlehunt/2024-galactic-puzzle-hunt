...
window.addEventListener("load", () => {
  ...
  defaultHuntFontColor = bodyStyles.getPropertyValue('color');
  bootstrapClipoardButton();
});
...
function handleClipboardButton() {
  ...
  // HACK GPH 2024: Use the puzzle element's styles instead of the
  // body styles for the defaults, since they may differ.
  const puzzleElementStyles = window.getComputedStyle(puzzleElement);
  defaultHuntFontFamily = puzzleElementStyles.getPropertyValue("font-family");
  defaultHuntFontSize = puzzleElementStyles.getPropertyValue('font-size');
  defaultHuntFontColor = puzzleElementStyles.getPropertyValue('color');
  ...
  try { copyContents(puzzleElement, clipboardButton.dataset); }
  catch(e) { clipboardButton.textContent = '❌'; console.error(e); }
  setTimeout(() => { clipboardButton.textContent = "📋️"; }, 3000);
}

function bootstrapClipoardButton() {
  ...
  document.addEventListener('copy', event => {
    if (!document.querySelector(".clipboard-button")) return;
    const puzzleElement = document.querySelector(".clipboard-container");
    ...
    // GPH 2024: Not all puzzles have plain text.
    // if (!plainTextVersion) {
    //   document.querySelector(".clipboard-button").textContent = '❌';
    // }
...
function copyJackInlineStyles(element) {
  ...
  // HACK GPH 2024: Detect default colors by direct matching.
  // This won't work if the color is switched to something and back
  // again, but that case should be rare especially when we're using
  // a non-black color.
  // Same for font size.
  const isDefaultColor = styles.getPropertyValue('color') === defaultHuntFontColor;
  const isDefaultFontSize = styles.getPropertyValue('font-size') === defaultHuntFontSize;
  const jackedStyles = [
...
function maybeAppendResetSpan(element) {
  ...
  // HACK GPH 2024: This is in copyjack since sheets doesn't properly
  // reset styles after an inline element.
  // For us, we're avoiding overwriting the font family and size for
  // everything, since the site defaults don't work well in sheets,
  // so skip setting those here.
  // We don't play with different fonts and sizes intentionally
  // in any of our puzzles this year anyway.
...
function copyJackInlineBorders(element) {
  ...
  if (borderStylesText) {
    element.dataset.copyOnlyStyles = borderStylesText + (element.dataset.copyOnlyStyles || '');
  }
}
